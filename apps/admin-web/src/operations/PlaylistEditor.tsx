...
type Item = { key: number; contentVersionId: string; durationMilliseconds: number; loopVideo: boolean }
...
export default function PlaylistEditor({ contents, tenantId, post, onCreated }: {
  ...
  const [items, setItems] = useState<Item[]>([{ key: 0, contentVersionId: '', durationMilliseconds: 10000, loopVideo: false }])
  ...
  return <form className="stack-form" onSubmit={(event) => {
    ...
    void post<Playlist>(`/api/v1/tenants/${tenantId}/playlists`, {
      ...
      items: items.map(({ contentVersionId, durationMilliseconds, loopVideo }) => ({ contentVersionId, durationMilliseconds, loopVideo })),
    }).then((created) => {
      onCreated(created); form.reset()
      setItems([{ key: 0, contentVersionId: '', durationMilliseconds: 10000, loopVideo: false }])
    }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Création impossible.'))
      .finally(() => setBusy(false))
  }}><h3>Créer une playlist</h3>
    <p>Enregistrez le brouillon, puis publiez-le depuis la liste.</p>
    ...
    {items.map((item, index) => <fieldset key={item.key} disabled={busy}><legend>Élément {index + 1}</legend>
      <label>Contenu<select required value={item.contentVersionId} onChange={(event) => change(item.key, { contentVersionId: event.target.value, loopVideo: false })}>
    ...
    <button type="button" disabled={busy || items.length >= contents.length || items.length >= 100} onClick={() => setItems((current) => [...current, { key: Math.max(...current.map((item) => item.key)) + 1, contentVersionId: '', durationMilliseconds: 10000, loopVideo: false }])}>Ajouter un élément</button>
    <button className="primary-button" disabled={busy || contents.length === 0} type="submit">Enregistrer le brouillon</button>
  </form>
}
