...
| Entity Framework Core / `dotnet-ef` | 10.0.7 | Data model, migrations and PostgreSQL persistence | MIT; Microsoft | Tool manifest and central NuGet pins; migrations are reviewed |
| Npgsql EF provider / driver | 10.0.3 | PostgreSQL 18 connectivity | PostgreSQL licence; Npgsql | EF Relational is centrally aligned to 10.0.7 |
...
| Testcontainers PostgreSQL | 4.14.0 | Real database boundary tests | MIT; NuGet | Docker access only in test environment |
| PostgreSQL image | 18.4-alpine3.24 | Local/integration database | PostgreSQL licence; official image | Loopback port; local password required |
